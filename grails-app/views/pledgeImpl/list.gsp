	...
	<body>
		...
		<div id="list-pledgeImpl" class="content scaffold-list" role="main">
			...
			<table>
				...
				<g:each in="${pledgeImplInstanceList}" status="i" var="pledgeImplInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${pledgeImplInstance.id}">${fieldValue(bean: pledgeImplInstance, field: "amount")}</g:link></td>
					
						<td>${fieldValue(bean: pledgeImplInstance, field: "member")}</td>
			...
			<div class="pagination">
				<g:paginate total="${pledgeImplInstanceTotal}" />
			</div>
		</div>
	</body>
</html>
