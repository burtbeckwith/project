...
<div class="fieldcontain ${hasErrors(bean: titheInstance, field: 'member', 'error')} required">
	...
	<g:select id="member" name="member.id" from="${edu.harvard.cscie56.Members.list()}" optionKey="id" optionValue="fullname"required="" optionValue="fullname"value="${titheInstance?.member?.id}" class="many-to-one"/>
</div>
